...
def _apply_xsa(attn_out: torch.Tensor, self_values: torch.Tensor) -> torch.Tensor:
    """Project attn_out orthogonal to self_values (removes the self-attention residual)."""
    out_f = attn_out.float()
    vs_f = self_values.float()
    denom = vs_f.pow(2).sum(dim=-1, keepdim=True).clamp_min(torch.finfo(vs_f.dtype).eps)
    proj = (out_f * vs_f).sum(dim=-1, keepdim=True) / denom
    return (out_f - proj * vs_f).to(attn_out.dtype)
...
class MultiSelfAttentionHeadVarlenFlex(torch.nn.Module):
    ...
    def forward(self, x, x_lens=None):
        ...
        if self.use_xsa:
            # vs: [1, num_heads, seq, dim_head] → [seq, num_heads, dim_head]
            outs = _apply_xsa(outs, vs.transpose(1, 2).squeeze(0))
...
class MultiSelfAttentionHeadLocal(torch.nn.Module):
    ...
    def forward(self, x, coords=None, ada_ln_aux=None):
        ...
        if self.use_xsa:
            # outs: [batch, seq, heads, dim_head]; vs after transpose: [batch, seq, heads, dim_head]
