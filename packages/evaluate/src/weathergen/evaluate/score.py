...
class Scores:
    ...
    def get_2x2_event_counts(
        ...
        """
        Get counts of 2x2 contingency tables (a: hits, b: false alarms,
        c: misses, d: correct negatives).
        """
        # boolean masks first (pure DataArray ops)
        hit_mask   = (p >= thresh) & (gt >= thresh)
        fa_mask    = (p >= thresh) & (gt <  thresh)
        miss_mask  = (p <  thresh) & (gt >= thresh)
        cn_mask    = (p <  thresh) & (gt <  thresh)

        if group_by_coord:
            a = hit_mask.groupby(group_by_coord).sum(dim=self._agg_dims)
            b = fa_mask.groupby(group_by_coord).sum(dim=self._agg_dims)
            c = miss_mask.groupby(group_by_coord).sum(dim=self._agg_dims)
            d = cn_mask.groupby(group_by_coord).sum(dim=self._agg_dims)
        else:
            a = hit_mask.sum(dim=self._agg_dims)
            b = fa_mask.sum(dim=self._agg_dims)
            c = miss_mask.sum(dim=self._agg_dims)
            d = cn_mask.sum(dim=self._agg_dims)
