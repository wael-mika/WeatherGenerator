...
from weathergen.evaluate.score import get_score, VerifiedData

_logger = logging.getLogger(__name__)

_REPO_ROOT = Path(__file__).parent.parent.parent.parent.parent  # TODO use importlib for resources
...
def calc_scores_per_stream(zio: ZarrIO, stream: str, metrics: list[str], channels: str | list[str] = None):
    """
    ...
    channels_stream = [ch for ch in channels_stream if ch in to_list(channels)] if channels else channels_stream

    # initialize the DataArray to store metrics
    metric_stream = xr.DataArray(np.full((nforecast_steps, len(channels_stream), nmetrics), np.nan), 
                                 coords={"forecast_step": forecast_steps, "channel": channels_stream, "metric": metrics})
    ...
    for fstep in forecast_steps:
        ...
        _logger.debug(f"Concatenating targets and predictions for stream {stream}, forecast_step {fstep}...")
        targets_all, preds_all = xr.concat(targets, dim="ipoint"), xr.concat(preds, dim="ipoint")
        ...
        _logger.debug(f"Build computation graphs for metrics for stream {stream}, forecast_step {fstep}...")
        combined_metrics = [get_score(score_data, metric, agg_dims="ipoint") for metric in metrics]
...
def metric_list_to_dict(metric_list: list[xr.DataArray], streams: list):
    ...
    assert len(metric_list) == len(streams), f"Inconsistent list of metrics and streams passed."
            ...
            for ch_idx, ch in enumerate(channels):
                ...
                for step_idx, step in enumerate(forecast_steps):
                    ...
                    for metric_idx, metric in enumerate(metrics):
                        metric = str(metric)
                        value = float(da_metric.isel({"metric": metric_idx, "forecast_step": step_idx, "channel": ch_idx}).values)
...
def fast_evaluation(run_id: str, metrics: list[str], save_dir: Path, results_dir: Path = _DEFAULT_RESULT_PATH, 
                    streams: str | list[str] = None, channels: str | list[str] = None, epoch: int = 0, rank: int = 0):
    """
    ...
    results_zarr = results_dir / run_id / f"validation_epoch{epoch:05d}_rank{rank:04d}.zarr"
    ...
    _logger.info(f"Finished computing metric scores for all streams. Total streams processed: {len(all_metric_streams)}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fast evaluation of weather generator runs.")

    parser.add_argument("-id", "--run_id", type=str, help="The ID of the run to evaluate.")
    parser.add_argument("-m", "--metrics", type=str, nargs='+', default=["rmse", "mae", "bias"], 
                        help="List of metrics to evaluate.")
    parser.add_argument("-sd", "--save_dir", type=Path, default=None, 
                        help="Directory to save the results.")
    parser.add_argument("-rd", "--results_dir", type=Path, default=_DEFAULT_RESULT_PATH, 
                        help="Directory containing the results zarr files.")
    parser.add_argument("-s", "--streams", type=str, nargs='*', default=None,
                        help="List of streams to evaluate.")
    parser.add_argument("-c", "--channels", type=str, nargs='*', default=None, 
                        help="List of channels to evaluate.")
    parser.add_argument("-e", "--epoch", type=int, default=0, 
                        help="Epoch number of inference run.")
    parser.add_argument("-r", "--rank", type=int, default=0, 
                        help="Rank of inference run.")
    ...
    fast_evaluation(args.run_id, args.metrics, save_dir, args.results_dir, args.streams, args.channels, args.epoch, args.rank) 